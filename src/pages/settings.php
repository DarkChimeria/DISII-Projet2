...
// }else{
  ...
  else
  {
   ...
   if(in_array($imgExt, $valid_extensions)){   
    ...
    $errMSG = $errMSG = '<div class="box-body"><div class="alert alert-warning alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h4><i class="icon fa fa-check"></i> Attention !</h4>Seuls les images de type : JPEG, JPG, PNG, GIF sont acceptées</div></div>';  
   }
  }

if(!isset($errMSG))
  {
    $sql = "UPDATE settings SET name_website=?,description_website=?,mail_website=?,adress_website=?,phone_website=?,domain=?,logo_website=?,idRNA=?,price_license=? WHERE id=1";
    $idRequete = executeR($connexion,$sql,array($name,$description,$mail,$adress,$phone,$domain,$userpic,$rna,$price));
